...
{
    [RoutePrefix("api/jobfinbillnotgenerated")]
    public class JobFinBillNotGeneratedController : ApiController
    {
        private readonly JobFinBillNotGeneratedDAL _dal = new JobFinBillNotGeneratedDAL();

        private static readonly string[] DateFormats = { "yyyy/MM/dd", "yyyy-MM-dd" };

        // QUERY: /api/jobfinbillnotgenerated/report?fromDate=2026/01/01&toDate=2026/01/31&costCtr=511.20
        [HttpGet]
        [Route("report")]
        ...
        private IHttpActionResult ExecuteQuery(string fromDate, string toDate, string costCtr)
        {
            try
            {
                ...
                var data = _dal.GetJobFinBillNotGenerated(fromDt, toDt, costCtr.Trim());
