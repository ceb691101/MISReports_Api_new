...
{
    [RoutePrefix("api/ccgrnnotgen")]
    public class CcGrnNotGenController : ApiController
    {
        private readonly CcGrnNotGenDAL _dal = new CcGrnNotGenDAL();

        // PATH: /api/ccgrnnotgen/report/2026-01-01/2026-07-31/510.00
        [HttpGet]
        ...
        // QUERY: /api/ccgrnnotgen/report?fromDate=2026-01-01&toDate=2026-07-31&costCtr=510.00
        [HttpGet]
        [Route("report")]
        ...
        private IHttpActionResult ExecuteQuery(string fromDate, string toDate, string costCtr)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(toDate) || !DateTime.TryParse(toDate, out DateTime toDt))
                    return BadRequest("toDate must be a valid date (e.g., 2026-07-31).");
                if (toDt < fromDt)
                ...
                var data = _dal.GetCcGrnNotGen(fromDateFormatted, toDateFormatted, costCtr.Trim());
