...
{
    public class MaterialReqWithIssueDAL
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public List<MaterialReqWithIssueModel> GetMaterialReqWithIssue(string fromDate, string toDate, string costCtr, string matCode)
        {
            var result = new List<MaterialReqWithIssueModel>();

            string query = @"
        SELECT 'Material Requistion' AS category,
               A.doc_no,
               A.req_dt AS trx_dt, B.res_cd AS mat_cd, B.req_units,
               A.issue_doc_no, A.req_source, A.ref_1, A.apr_uid_1, A.qty_apr_uid AS apr_uid_2,
               A.apr_dt_1, A.qty_apr_dt AS apr_dt_2,
               (SELECT T1.trx_dt FROM inpostmt T1, intrhmt T2
        ...
        SELECT 'Cost Center' AS category,
               A.doc_no,
               A.req_dt AS trx_dt, B.res_cd AS mat_cd, B.req_units,
               A.issue_doc_no, '' AS req_source, '' AS ref_1, A.apr_uid_1, A.qty_apr_uid AS apr_uid_2,
               A.apr_dt_1, A.qty_apr_dt AS apr_dt_2,
               (SELECT T1.trx_dt FROM inpostmt T1, intrhmt T2
                WHERE T1.doc_no = T2.doc_no
                  AND T1.doc_pf = T2.doc_pf
        ...
        ORDER BY 1, 4, 7, 2";
            ...
            using (var cmd = new OracleCommand(query, conn))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        result.Add(new MaterialReqWithIssueModel
                        {
                            ...
                            IssuedReturnQty = reader["Issued_return_qty"] == DBNull.Value ? (decimal?)null : Convert.ToDecimal(reader["Issued_return_qty"]),
