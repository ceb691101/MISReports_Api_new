...
{
    public class CurrAcctReconExtPeriodRepository
    {
        private readonly string connectionString = ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public List<CurrAcctReconExtPeriodModel> GetData(string compId, int repyear, int monthFrom, int monthTo, string subac)
        {
            var result = new List<CurrAcctReconExtPeriodModel>();

            string sql = @"
select     T2.ent_dt as EntDt, T2.acct_dt as AcctDt, T2.post_dt as PostDt,
            T1.sub_ac as SubAc,
...
(select     sum(T4.op_bal)
 from       glsubbal T4
 where      substr(T4.gl_cd,8,5) =  'L9200'  and
           trim(T4.sub_ac) = trim(:SUBAC) and
            T4.yr_ind=  :REPYEAR and T4.mth_ind=  :MONTHFROM and t4.dept_id in  (select dept_id  from gldeptm  where status=2 and comp_id in (select comp_id
				  	      	from glcompm
...
(select     sum(T4.cl_bal)
 ...
 where      substr(T4.gl_cd,8,5) =  'L9200'  and
           trim(T4.sub_ac) = trim(:SUBAC) and
            T4.yr_ind=  :REPYEAR and T4.mth_ind= :MONTHTO and t4.dept_id in  (select dept_id  from gldeptm  where status=2 and comp_id in (select comp_id
				  	      	from glcompm
				              	where trim(comp_id) = trim(:COMPID) or trim(parent_id) = trim(:COMPID)
...
(select comp_nm from glcompm where trim(comp_id) = trim(:COMPID)) as CompNm
 ...
 where      T1.doc_no=T2.doc_no and
            ...
            substr(T1.gl_cd,8,5) =  'L9200'  and
            trim(T1.sub_ac) = trim(:SUBAC) and
            T2.status = 6 and
            T2.log_yr = :REPYEAR and
            T2.log_mth BETWEEN :MONTHFROM AND :MONTHTO and
...
ORDER   BY    T2.post_dt, T1.doc_no";
            ...
            using (OracleConnection con = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand(sql, con))
                {
                    ...
                    cmd.Parameters.Add(new OracleParameter("MONTHFROM", OracleDbType.Int32)).Value = monthFrom;
                    cmd.Parameters.Add(new OracleParameter("MONTHTO", OracleDbType.Int32)).Value = monthTo;
                    ...
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var item = new CurrAcctReconExtPeriodModel
                            {
