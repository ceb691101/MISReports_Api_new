...
{
    public class PriceVaDAL
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public List<PriceVaModel> GetPriceVa(string repYear, string repMonth, string costCtr)
        {
            var result = new List<PriceVaModel>();
            const string query = @"
                SELECT DISTINCT T1.mat_cd, T1.grade_cd, T1.unit_price, T1.new_price,
                                (CASE WHEN T1.net_change > 0 THEN T1.net_change ELSE 0 END) AS I_net_change,
                                (CASE WHEN T1.net_change < 0 THEN T1.net_change ELSE 0 END) AS D_net_change,
                                T1.qty_on_hand,
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        result.Add(new PriceVaModel
                        {
                            ...
                            INetChange = reader["I_net_change"] == DBNull.Value ? (decimal?)null : Convert.ToDecimal(reader["I_net_change"]),
                            DNetChange = reader["D_net_change"] == DBNull.Value ? (decimal?)null : Convert.ToDecimal(reader["D_net_change"]),
