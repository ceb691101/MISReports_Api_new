    ...
    public class AreaT1T2T3DAL
    {
        ...
        public List<AreaT1T2T3Model> GetAreaT1T2T3(DateTime fromDate, DateTime toDate, string compId)
        {
                  ...
                  AND     EXISTS (
                              SELECT 1
                               ...
                               WHERE TRIM(g.dept_id) = TRIM(a.dept_id)
                                 AND TRIM(g.comp_id) = TRIM(:compid)
                          )
            ...
            using (var cmd = new OracleCommand(query, conn))
            {
                ...
                cmd.BindByName = true;
                cmd.CommandTimeout = 120; // give the wider filter room; tune as needed
                ...
                catch (OracleException ex)
                {
                    ...
                    throw; // rethrow so the API layer still returns an error, but now you'll have the ORA- code in logs
                }
            }
        ...
        private static decimal? SafeGetDecimal(OracleDataReader reader, int ordinal, string columnName, string applicationNoForLog)
        {
            ...
            catch (OverflowException)
            {
                System.Diagnostics.Trace.WriteLine(
                    $"[AreaT1T2T3DAL] Overflow converting column '{columnName}' for application_no '{applicationNoForLog}'. " +
                    $"Raw Oracle value: {oracleVal.ToString()}");
                return null; // swap for a sentinel/rethrow if you'd rather fail loudly than null it out
            }
        }
    }
}